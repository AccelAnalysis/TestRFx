...
function IntelligenceDetail({ record, notes }: { record: ExchangeRecord; notes: string[] }) {
  const intelligence = getIntelligenceDetail(record);
  if (!intelligence) return null;
  return <>
    <p>{record.summary}</p>
    <h2>Intelligence context</h2>
    <dl className="intelligence-detail-grid">
      <div><dt>Signal</dt><dd>{intelligence.signalType}</dd></div>
      <div><dt>Observed period</dt><dd>{intelligence.observedPeriod}</dd></div>
      <div><dt>Source</dt><dd>{intelligence.sourceLabel}</dd></div>
      <div><dt>Source class</dt><dd>{intelligence.sourceType.replaceAll("-", " ")}</dd></div>
    </dl>
    <div className="provenance-panel"><strong>Source & provenance</strong><p>{intelligence.provenance}</p></div>
    ...
    <h2>Notes</h2>
    {notes.length ? <div className="intelligence-notes">{notes.map((note, index) => <p key={`${record.id}-note-${index}`}>{note}</p>)}</div> : <p className="muted">No notes have been added in this reference session.</p>}
    <h2>Decision pathways</h2>
    <div className="intelligence-outcomes"><span>Decision support</span><span>Opportunity / capability matching</span><span>Referral trigger · shared workflow</span><span>Track / follow / return</span></div>
  </>;
}

export function DetailSurface({ record, actions, activeActionIds, notes = [], onAction, onClose }: {
  ...
  return (
    <section className="detail-surface" role="dialog" aria-modal="true" aria-label={`${record.title} details`}>
      ...
      <div className="detail-body">
        ...
        {record.type === "rfx" ? <RfxDetailContent recordId={record.id} /> : record.type === "resource" ? <ResourceDetail record={record} /> : record.type === "intelligence" ? <IntelligenceDetail record={record} notes={notes} /> : <><p>{record.summary}</p><h2>Record context</h2><div className="detail-tags">{record.metadata.map((item) => <span key={item}>{item}</span>)}</div></>}

...
export function DetailSurface({ record, actions, activeActionIds, notes = [], onAction, onReferResource, onClose }: { record: ExchangeRecord; actions: LensAction[]; activeActionIds?: string[]; notes?: string[]; onAction: (action: LensAction) => void; onReferResource?: (recordId: string) => void; onClose: () => void; }) {
  ...
  return <section className="detail-surface" role="dialog" aria-modal="true" aria-label={`${record.title} details`}>
    ...
    <div className="detail-body"><ActionRail actions={actions} activeActionIds={activeActionIds} onAction={onAction} />
      {record.type === "rfx" ? <RfxDetailContent recordId={record.id} /> : record.type === "resource" ? <ResourceDetail record={record} onRefer={onReferResource ? () => onReferResource(record.id) : undefined} /> : record.type === "intelligence" ? <IntelligenceDetail record={record} notes={notes} /> : capabilityProfile ? <CapabilityDetailBody profile={capabilityProfile} /> : <><p>{record.summary}</p><h2>Record context</h2><div className="detail-tags">{record.metadata.map((item) => <span key={item}>{item}</span>)}</div></>}

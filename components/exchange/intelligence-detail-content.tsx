...
import { IntelligenceActivityPanel } from "./intelligence-activity-panel";
...
export function IntelligenceDetailContent({ recordId, refreshKey = 0 }: { recordId: string; refreshKey?: number }) {
  ...
  useEffect(() => {
    ...
    getIntelligenceRecord(recordId)
      ...
      .catch((caught) => { if (!cancelled) { setDetail(undefined); setError(caught instanceof IntelligenceServiceError ? caught.message : "The Intelligence record could not be loaded."); } })
  ...
  return <>
    ...
    <div className="provenance-panel">
      ...
      {detail.sources.length ? <div className="intelligence-source-list">{detail.sources.map((source) => <article key={source.id}><strong>{source.label}</strong><span>{source.type.replaceAll("-", " ")}</span>{source.publisher ? <span>{source.publisher}</span> : null}{source.retrievedAt ? <small>Retrieved {new Date(source.retrievedAt).toLocaleString()}</small> : null}{source.uri ? <a href={source.uri} target="_blank" rel="noreferrer">Open source ↗</a> : null}</article>)}</div> : <p>No source records are attached.</p>}
    ...
    <h2>Activity</h2>
    <IntelligenceActivityPanel recordId={recordId} refreshKey={refreshKey} />
  </>;
}

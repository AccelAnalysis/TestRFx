...
export function IntelligenceNavigationSurface({ nodeId, record, onNavigate, onBack, onClose }: { nodeId: string; record?: ExchangeRecord; onNavigate: (nodeId: string) => void; onBack: () => void; onClose: () => void; }) {
  ...
  if (!node) return null;
  async function track(mode: "track" | "follow") { if (!record) return; setPending(true); setMessage(""); try { await setIntelligenceTracking(record.id, mode, true); setMessage(mode === "track" ? "Tracking enabled." : "Following enabled."); } catch (error) { setMessage(error instanceof Error ? error.message : "Tracking could not be updated."); } finally { setPending(false); } }

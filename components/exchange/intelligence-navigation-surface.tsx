...
export function IntelligenceNavigationSurface({ nodeId, record, onNavigate, onBack, onClose, onRunWorkflow }: { nodeId: string; record?: ExchangeRecord; onNavigate: (nodeId: string) => void; onBack: () => void; onClose: () => void; onRunWorkflow: (workflow: IntelligenceWorkflow, record?: ExchangeRecord) => void; }) {
  ...
  async function track(mode: "track" | "follow", advance = false) { if (!record) return; setPending(true); setMessage(""); try { await setIntelligenceTracking(record.id, mode, true); setMessage(mode === "track" ? "Tracking enabled." : "Following enabled."); const nextNodeId = currentNode.children?.[0]?.id; if (advance && nextNodeId) onNavigate(nextNodeId); } catch (error) { setMessage(error instanceof Error ? error.message : "Tracking could not be updated."); } finally { setPending(false); } }
  ...
  const showChildren = currentNode.kind === "root" || currentNode.kind === "view" || currentNode.kind === "result" || currentNode.outcomeAction === "referral-trigger";
  const actionWorkflow: IntelligenceWorkflow | undefined = currentNode.id === "intelligence.own.add" ? "add" : currentNode.id === "intelligence.own.edit" ? "edit" : currentNode.id === "intelligence.other.note" ? "note" : currentNode.id === "intelligence.own.compare" || currentNode.id === "intelligence.other.compare" ? "compare" : undefined;
  const directTrack = currentNode.id === "intelligence.own.track" ? "track" : currentNode.id === "intelligence.other.follow" ? "follow" : undefined;

  return <section className="intelligence-nav-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="intelligence-nav-surface" role="dialog" aria-modal="true" aria-label={currentNode.label}>
    <header className="intelligence-nav-header"><button type="button" onClick={path.length > 1 ? onBack : onClose}>←</button><div><p className="eyebrow">Intelligence</p><h2>{currentNode.label}</h2></div><button type="button" onClick={onClose}>×</button></header>
    <nav className="intelligence-breadcrumbs" aria-label="Intelligence hierarchy">{path.map((item, index) => <span key={item.id}>{index ? <i aria-hidden>›</i> : null}<button type="button" onClick={() => onNavigate(item.id)} aria-current={item.id === currentNode.id ? "page" : undefined}>{item.label}</button></span>)}</nav>
    <p className="intelligence-nav-description">{currentNode.description}</p>{record ? <div className="intelligence-nav-subject"><small>Selected intelligence</small><strong>{record.title}</strong><span>{record.organization} · {record.geography}</span></div> : null}
    {showChildren ? <div className="intelligence-nav-list">{currentNode.children?.map((child) => <button type="button" className="intelligence-nav-node" key={child.id} onClick={() => onNavigate(child.id)}><span><strong>{child.label}</strong><small>{child.description}</small></span><span aria-hidden>›</span></button>)}</div> : null}
    {actionWorkflow ? <div className="intelligence-nav-action"><button className="workflow-primary" type="button" disabled={actionWorkflow !== "add" && !record} onClick={() => onRunWorkflow(actionWorkflow, record)}>{currentNode.label}</button></div> : null}
    {directTrack ? <div className="intelligence-nav-action"><button className="workflow-primary" type="button" disabled={!record || pending} onClick={() => void track(directTrack, true)}>{pending ? "Saving…" : currentNode.label}</button></div> : null}
    {currentNode.id === "intelligence.other.view" && record ? <button className="workflow-primary" type="button" onClick={() => onNavigate(currentNode.children?.[0]?.id ?? currentNode.id)}>View detail</button> : null}
    {(currentNode.outcomeAction === "decision-support" || currentNode.id.endsWith("view.context") || currentNode.id.endsWith("track.activity") || currentNode.id.endsWith("follow.monitor")) && record ? <IntelligenceDetailContent recordId={record.id} /> : null}
    {currentNode.outcomeAction === "matching" ? <div className="intelligence-match-panel">{pending ? <p>Loading related records…</p> : null}{matches.map((match) => <div className="intelligence-match" key={`${match.lens}:${match.recordId}`}><span><small>{match.lens === "rfx" ? "RFx" : "Capability"}</small><strong>{match.title}</strong><em>{match.organization} · {match.reason}</em></span></div>)}{!pending && !matches.length ? <p>No explicit related RFx or capability records are recorded. RFxchange will not invent a match.</p> : null}</div> : null}
    {currentNode.outcomeAction === "create-referral" ? <div className="intelligence-referral-form"><label>Receiving organization<input value={query} onChange={(event) => { setRecipient(undefined); setQuery(event.target.value); }} /></label>{organizations.map((organization) => <button type="button" key={organization.id} onClick={() => { setRecipient(organization); setQuery(organization.name); setOrganizations([]); }}>{organization.name}</button>)}{recipient ? <div className="provenance-panel"><strong>Referral policy</strong><p>{policy?.published ? policy.policySummary || "No additional policy text." : "No published policy or fee terms. None will be invented."}</p>{policy?.feeSummary ? <p>{policy.feeSummary}</p> : null}</div> : null}<label>Referral note<textarea value={note} onChange={(event) => setNote(event.target.value)} /></label><button className="workflow-primary" type="button" disabled={!recipient || pending || (policy?.published === true && !policy.active)} onClick={() => void refer()}>{pending ? "Creating…" : "Create Referral"}</button></div> : null}
    {currentNode.outcomeAction === "return-exchange" && record ? <div className="intelligence-nav-action"><button className="workflow-primary" type="button" disabled={pending} onClick={() => void track(record.ownedByViewer ? "track" : "follow")}>{pending ? "Saving…" : record.ownedByViewer ? "Track intelligence" : "Follow intelligence"}</button><button type="button" onClick={onClose}>Return to Exchange</button></div> : null}

...
import type { Coordinates, DrawerQueryState, DrawerState, ExchangeFilters, ExchangeLens, ExchangeSearchState, GeolocationStatus, MapDisplayMode, MapViewState, RecentSearch, SavedSearch } from "@/lib/exchange/contracts";
...
function initialSearchStates() { return Object.fromEntries(lensOrder.map((lens) => [lens, defaultSearchState()])) as Record<ExchangeLens, ExchangeSearchState>; }
function initialFloatingFilters() { return Object.fromEntries(lensOrder.map((lens) => [lens, createExchangeFilters()])) as Record<ExchangeLens, ExchangeFilters>; }
function initialDrawerQueries() { return Object.fromEntries(lensOrder.map((lens) => [lens, createDefaultDrawerQuery()])) as Record<ExchangeLens, DrawerQueryState>; }
...
export function ExchangeShell({ initialLens = "rfx", initialRecordId }: { initialLens?: ExchangeLens; initialRecordId?: string }) {
  ...
  useEffect(() => { try { const recent = window.localStorage.getItem(recentStorageKey); const saved = window.localStorage.getItem(savedStorageKey); if (recent) setRecentSearches(JSON.parse(recent) as RecentSearch[]); if (saved) setSavedSearches(JSON.parse(saved) as SavedSearch[]); } catch {} }, []);
  useEffect(() => {
    function syncFromUrl() {
      const parts = window.location.pathname.split("/").filter(Boolean); const urlLens = parts[1];
      if (urlLens === "rfx" || urlLens === "resources" || urlLens === "intelligence" || urlLens === "capabilities") {
        const urlState = searchStateFromParams(new URLSearchParams(window.location.search)); setLens(urlLens); setSearchByLens((current) => ({ ...current, [urlLens]: urlState })); const recordId = parts[2]; setSelectedRecordId(recordId); setDetailRecordId(recordId);
      }
    }
  ...
  function commitSearch(next: ExchangeSearchState) { updateSearchState(next); if (!next.query.trim() && activeFilterCount(next) === 0) return; const recent: RecentSearch = { id: `${lens}-${Date.now()}`, lens, state: next, createdAt: new Date().toISOString() }; const deduped = recentSearches.filter((item) => item.lens !== lens || JSON.stringify(item.state) !== JSON.stringify(next)); persistRecent([recent, ...deduped].slice(0, 12)); }
  ...
  function changeLens(next: ExchangeLens) { const prior = searchByLens[next]; const carried = prior.query.trim() ? prior : { ...prior, query: searchState.query }; setLens(next); setSearchByLens((current) => ({ ...current, [next]: carried })); setSelectedRecordId(undefined); setDetailRecordId(undefined); setUrl(next, undefined, "replace", carried); }
  ...
  function updateDrawerQuery(next: DrawerQueryState) { setDrawerQueries((current) => ({ ...current, [lens]: next })); }
  ...
  function locateViewer() { if (!("geolocation" in navigator)) { setGeolocationStatus("unavailable"); return; } setGeolocationStatus("requesting"); navigator.geolocation.getCurrentPosition((position) => { setViewerLocation({ lat: position.coords.latitude, lng: position.coords.longitude }); setGeolocationStatus("located"); }, (error) => setGeolocationStatus(error.code === 1 ? "denied" : "unavailable"), { enableHighAccuracy: false, timeout: 8000, maximumAge: 60000 }); }

  const lensRecent = recentSearches.filter((item) => item.lens === lens); const lensSaved = savedSearches.filter((item) => item.lens === lens); const mapped = records.filter((record) => record.location).length; const offMap = records.length - mapped; const resultContext = `${mapped} mapped · ${offMap} off-map`;

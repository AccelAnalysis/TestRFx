...
import type { Coordinates, DrawerQueryState, DrawerState, ExchangeFilters, ExchangeLens, ExchangeRecord, ExchangeSearchState, GeolocationStatus, LensAction, MapViewState, RecentSearch, SavedSearch } from "@/lib/exchange/contracts";
import { exchangeSeed } from "@/lib/exchange/seed";
import { applyExchangeFilters, countActiveFilters, createExchangeFilters, normalizeExchangeFilters } from "@/lib/exchange/filter";
...
import { createDefaultMapView, filterRecordsToMapBounds, fitMapCameraToRecords, mapBoundsForCamera } from "@/lib/exchange/map-model";
...
const floatingFilterStorageKey = "rfxchange:floating-filters";
...
export function ExchangeShell({ initialLens = "rfx", initialRecordId }: { initialLens?: ExchangeLens; initialRecordId?: string }) {
  ...
  const areaRecords = useMemo(() => filterRecordsToMapBounds(filteredRecords, mapView.queriedBounds), [filteredRecords, mapView.queriedBounds]);
  const records = useMemo(() => applyDrawerQuery(areaRecords, drawerQuery), [areaRecords, drawerQuery]);
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem(floatingFilterStorageKey);
      if (stored) {
        const parsed = JSON.parse(stored) as Partial<Record<ExchangeLens, unknown>>;
        setFiltersByLens(Object.fromEntries(lensOrder.map((item) => [item, normalizeExchangeFilters(parsed[item])])) as Record<ExchangeLens, ExchangeFilters>);
      }
    } catch {}
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem(floatingFilterStorageKey, JSON.stringify(filtersByLens)); } catch {}
  }, [filtersByLens, filtersHydrated]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setResourceNotice(undefined), 3600);
    return () => clearTimeout(timeout);
  }, [resourceNotice]);
  ...
  function createResource(draft: ResourceDraft) {
    const id = `res-local-${Date.now()}`;
    const resource = { category: draft.category, availability: draft.availability, availabilityLabel: draft.availabilityLabel, capacity: draft.capacity || undefined, serviceArea: draft.serviceArea || undefined, visibility: draft.visibility, terms: draft.terms || undefined, status: "active" as const };
    const record: ExchangeRecord = { id, type: "resource", title: draft.title, organization: "Your Organization", summary: draft.summary, geography: draft.geography, metadata: ["Owned by you", ...resourceMetadata(resource)], location: draft.visibility === "public-location" ? { lat: 36.9, lng: -76.71 } : undefined, ownedByViewer: true, card: { eyebrow: "Resource Offer", classifications: [draft.category], status: { label: draft.availabilityLabel, tone: "success" }, relationships: ["owned"], distance: "Local" }, resource };
    setRecordsState((current) => [record, ...current]); setSelectedRecordId(id); setResourceWorkflow(undefined); setResourceNotice("Resource offer published to the reference Exchange. Production persistence remains behind the Resources service boundary.");
  }
  function updateResource(recordId: string, draft: ResourceDraft) {
    setRecordsState((current) => current.map((record) => {
      if (record.id !== recordId || !record.resource) return record;
      const resource = { ...record.resource, category: draft.category, availability: draft.availability, availabilityLabel: draft.availabilityLabel, capacity: draft.capacity || undefined, serviceArea: draft.serviceArea || undefined, visibility: draft.visibility, terms: draft.terms || undefined };
      return { ...record, title: draft.title, summary: draft.summary, geography: draft.geography, metadata: ["Owned by you", ...resourceMetadata(resource)], location: draft.visibility === "public-location" ? (record.location ?? { lat: 36.9, lng: -76.71 }) : undefined, card: { ...record.card, classifications: [draft.category], status: { label: draft.availabilityLabel, tone: "success" } }, resource };
    }));
    setResourceWorkflow(undefined); setResourceNotice("Resource changes saved in the reference Exchange.");
  }
  function requestResource(recordId: string, request: ResourceRequestDraft) {
    const record = allRecords.find((item) => item.id === recordId); setResourceWorkflow(undefined); setResourceNotice(`Request created for ${record?.title ?? "resource"}${request.neededBy ? ` · needed ${request.neededBy}` : ""}. Provider notification and fulfillment remain shared-service integrations.`);
  }
  ...
  async function handleAction(action: LensAction, record?: ExchangeRecord) {
    ...
    if (action.id === "share" && record) {
      ...
      try { if (navigator.share) await navigator.share({ title: record.title, text: record.summary, url }); else { await navigator.clipboard.writeText(url); setActionNotice("Record link copied."); } }
      catch (error) { if (error instanceof DOMException && error.name === "AbortError") return; setActionNotice("Unable to share this record right now."); }
      return;
    }
    if (action.toggle && record) {
      if (action.toggle === "save") toggleSaved(record.id);
      else { const key = actionKey(record, action); const next = !actionIsActive(record, action); setActionState((current) => ({ ...current, [key]: next })); }
      setActionNotice(`${action.label}: ${record.title}`);
    }
  }

  function resetMapView() { setMapView(createDefaultMapView()); setViewportDirty(false); }
  function resetBearing() { setMapView((current) => ({ ...current, camera: { ...current.camera, bearing: 0 } })); }
  function locateViewer() {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      setMapView((current) => ({
        ...
        camera: { ...current.camera, center: coordinate, zoom: Math.max(current.camera.zoom, 11.2) },
      }));
      setViewportDirty(false);
      setGeolocationStatus("located");
    }, (error) => setGeolocationStatus(error.code === 1 ? "denied" : "unavailable"), { enableHighAccuracy: true, timeout: 8000, maximumAge: 60000 });
  }
  function searchCurrentMapArea() {
    setMapView((current) => ({ ...current, queriedBounds: mapBoundsForCamera(current.camera) }));
    setViewportDirty(false);
  }
  ...
  function fitCurrentResults() {
    const camera = fitMapCameraToRecords(mapView.camera, records);
    if (!camera) { setActionNotice("No mapped results are available to fit."); return; }
    setMapView((current) => ({ ...current, camera }));
    setViewportDirty(false);
  }
  function recenterSelectedGeography() {
    const geography = floatingFilters.geography;
    if (!geography) return;
    const geographyRecords = searchRecords.filter((record) => record.geography === geography);
    const camera = fitMapCameraToRecords(mapView.camera, geographyRecords);
    if (!camera) { setActionNotice(`${geography} has no mapped records in this lens.`); return; }
    setMapView((current) => ({
      ...current,
      queriedBounds: undefined,
      geography: { id: `filter:${geography.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, label: geography, scope: "selected" },
      camera,
    }));
    setViewportDirty(false);
  }
  ...
  const areaQueryActive = Boolean(mapView.queriedBounds);
  const resultContext = areaQueryActive ? `${mapped} mapped · map area active` : `${mapped} mapped · ${offMap} off-map`;
  ...
  const canFitResults = visibleRecords.some((record) => Boolean(record.location));
  const canRecenterGeography = Boolean(floatingFilters.geography && searchRecords.some((record) => record.geography === floatingFilters.geography && record.location));
  ...
  return <main className="exchange-shell">
    ...
    <FloatingControls lens={lens} records={searchRecords.filter((record) => record.resource?.status !== "archived")} facetRecords={lensRecords.filter((record) => record.resource?.status !== "archived")} filters={floatingFilters} onFiltersChange={(next) => setFiltersByLens((current) => ({ ...current, [lens]: next }))} mapDisplayMode={mapView.camera.mode} mapBearing={mapView.camera.bearing} onMapDisplayModeChange={(mode) => setMapView((current) => ({ ...current, camera: { ...current.camera, mode, pitch: mode === "3d" ? 42 : 0 } }))} onResetBearing={resetBearing} geolocationStatus={geolocationStatus} onLocate={locateViewer} onResetView={resetMapView} onFitResults={fitCurrentResults} canFitResults={canFitResults} onRecenterGeography={recenterSelectedGeography} canRecenterGeography={canRecenterGeography} searchAreaAvailable={viewportDirty} areaQueryActive={areaQueryActive} onSearchArea={searchCurrentMapArea} onClearSearchArea={clearMapArea} filterRequestKey={filterRequestKey} />
    <ResultsDrawer state={drawer} onStateChange={setDrawer} lens={lens} lensLabel={definition.label} records={visibleRecords} totalAvailableCount={filteredRecords.length} selectedRecordId={selectedRecordId} actions={actions} activeActionIds={drawerActiveActionIds} onAction={(action) => { void handleAction(action, actionRecord); }} emptyMessage={definition.emptyMessage} resultContext={resultContext} query={drawerQuery} onQueryChange={(next) => setDrawerQueries((current) => ({ ...current, [lens]: next }))} onFilter={() => setFilterRequestKey((value) => value + 1)} activeFilterCount={countActiveFilters(floatingFilters)} onSelect={selectRecord} onOpen={openDetail} onToggleSave={toggleSaved} />

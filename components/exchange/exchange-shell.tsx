...
import {
  deriveMapGeographies,
  mapBoundsEqual,
  scopeMapRecordsToBounds,
  viewForCurrentLocation,
  viewForGeography,
...
import {
  ...
  createResourceReferral,
  listResourcesFromService,
  offerResourceThroughService,
  requestResourceThroughService,
  sendResourceThroughService,
  setSharedRelationshipThroughService,
...
export function ExchangeShell({
  ...
  const resolvedViewerContext = useMemo<ExchangeViewerContext>(
    () => ({ ...referenceViewerContext, ...viewerContext }),
    [referenceViewerContext, viewerContext],
  );
  ...
  const mapGeographies = useMemo(
    () => deriveMapGeographies(lensRecords.filter((record) => record.resource?.status !== "archived")),
    [lensRecords],
  );
  ...
  const resourceWorkflowRecord = resourceWorkflow && "recordId" in resourceWorkflow
    ...
    : undefined;
  const resourceNavigationRecord = allRecords.find((record) => record.id === (detailRecordId ?? selectedRecordId) && record.type === "resource");
  const intelligenceWorkflowRecord = intelligenceWorkflow?.recordId
    ...
    : undefined;
  const rawCapabilityWorkflowProfile = capabilityWorkflow
    ? getCapabilityProfileByExchangeRecordId(capabilityWorkflow.recordId)
    : undefined;
  const capabilityWorkflowProfile = rawCapabilityWorkflowProfile?.ownedByViewer
    ? { ...rawCapabilityWorkflowProfile, organizationName }
    : rawCapabilityWorkflowProfile;

  useEffect(() => {
    try {
      const recent = localStorage.getItem(recentStorageKey);
      const saved = localStorage.getItem(savedStorageKey);
      const localDrafts = localStorage.getItem(rfxDraftStorageKey);
  ...
  useEffect(() => {
    ...
    void listResourcesFromService()
      .then(({ records: liveResources }) => {
        if (!active) return;
        setRecordsState((current) => [
          ...current.filter((record) => record.type !== "resource"),
          ...liveResources,
        ]);
        setSavedRecordIds((current) => {
          const next = new Set(current);
          for (const record of liveResources) {
      ...
      .catch((error) => {
        if (!active) return;
        const isPagesPreview = typeof location !== "undefined" && location.hostname.endsWith("github.io");
        if (!isPagesPreview) {
          setRecordsState((current) => current.filter((record) => record.type !== "resource"));
          setResourceNotice(error instanceof Error ? error.message : "Resources runtime is unavailable.");
        }
      });
    return () => { active = false; };
  }, []);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => setResourceNotice(undefined), 5200);
    return () => clearTimeout(timeout);
  }, [resourceNotice]);
  ...
  function persistLocalRfxDraft(record: ExchangeRecord) {
    try {
      const raw = localStorage.getItem(rfxDraftStorageKey);
      const current = raw ? JSON.parse(raw) as ExchangeRecord[] : [];
      const next = [record, ...current.filter((item) => item.id !== record.id)];
      localStorage.setItem(rfxDraftStorageKey, JSON.stringify(next));
    } catch {}
  }
  ...
  function changeLens(next: ExchangeLens) {
    ...
    setActionNotice("");
    setMapView((current) => ({ ...current, queriedBounds: undefined }));
    setViewportDirty(false);
    setUrl(next, undefined, "replace", carried);
  }
  function selectRecord(id: string) {
    if (id === SELF_ORGANIZATION_ANCHOR_ID) return;
  ...
  function beginCreateRfx() {
    ...
    const draft: ExchangeRecord = {
      id,
      type: "rfx",
      title: "New RFx draft",
      organization: organizationName,
      summary: "RFx draft created from the issuer workflow.",
      geography: contextualGeography,
      metadata: [],
      ownedByViewer: true,
      card: { eyebrow: "RFx", status: { label: "Draft", tone: "neutral" } },
    };
    setRecordsState((current) => [draft, ...current]);
    persistLocalRfxDraft(draft);
    setSelectedRecordId(id);
    setDetailRecordId(undefined);
    setRfxWorkflow({ entry: "create-rfx", recordId: id });
  }
  ...
  async function toggleRfxWatch(record: ExchangeRecord) {
    ...
    setSavedRecordIds((current) => {
      ...
      if (currentlySaved) updated.delete(record.id); else updated.add(record.id);
      return updated;
    });
  ...
  async function setResourceRelationship(recordId: string, kind: "saved" | "following", active: boolean) {
    await setSharedRelationshipThroughService(recordId, kind, active);
    if (kind === "saved") {
      setSavedRecordIds((current) => {
        const next = new Set(current);
        if (active) next.add(recordId); else next.delete(recordId);
        return next;
      });
    }
    setRecordsState((current) => current.map((record) => {
      ...
      if (active) relationships.add(kind); else relationships.delete(kind);
      return {
        ...record,
        saved: kind === "saved" ? active : record.saved,
        card: { ...(record.card ?? {}), relationships: Array.from(relationships) },
      };
    }));
    setResourceWorkflow(undefined);
    setResourceNotice(`${active ? (kind === "saved" ? "Saved" : "Following") : (kind === "saved" ? "Removed from Saved" : "No longer following")} Resource.`);
  }

  function handleCardSave(id: string) {
    const record = allRecords.find((item) => item.id === id);
    if (record?.type === "rfx") { void toggleRfxWatch(record); return; }
    if (record?.type === "resource") {
      void setResourceRelationship(record.id, "saved", !savedRecordIds.has(record.id)).catch((error) => {
        setResourceNotice(error instanceof Error ? error.message : "Resource could not be saved.");
      });
      return;
    }
    toggleSaved(id);
  ...
  async function createResource(draft: ResourceDraft) {
    const { record } = await offerResourceThroughService(draft);
    setRecordsState((current) => [record, ...current.filter((item) => item.id !== record.id)]);
    if (record.saved) setSavedRecordIds((current) => new Set(current).add(record.id));
    setSelectedRecordId(record.id);
    setResourceWorkflow(undefined);
    setResourceNotice(record.location ? "Resource offer published with its canonical organization location." : "Resource offer published as an off-map/service-area listing; no coordinate was fabricated.");
  }

  async function updateResource(recordId: string, draft: ResourceDraft) {
    const { record } = await updateResourceThroughService(recordId, draft);
    setRecordsState((current) => current.map((item) => item.id === record.id ? record : item));
    setResourceWorkflow(undefined);
    setResourceNotice(record.location ? "Resource changes saved." : "Resource changes saved; the listing remains truthfully off-map.");
  }

  async function requestResource(recordId: string, request: ResourceRequestDraft) {
    await requestResourceThroughService(recordId, request);
    const record = allRecords.find((item) => item.id === recordId);
    setResourceWorkflow(undefined);
    setResourceNotice(`Request sent for ${record?.title ?? "Resource"}${request.neededBy ? ` · needed ${request.neededBy}` : ""}.`);
  }

  async function archiveResource(recordId: string) {
    await archiveResourceThroughService(recordId);
    ...
    setDetailRecordId(undefined);
    setResourceWorkflow(undefined);
    setResourceNotice("Resource archived and removed from active discovery.");
    setUrl("resources");
  }

  async function shareResource(recordId: string, recipientOrganizationId: string, message: string) {
    const { share } = await sendResourceThroughService(recordId, recipientOrganizationId, message);
    setResourceWorkflow(undefined);
    setResourceNotice(`Resource sent to ${share.recipientOrganization.name}.`);
  }

  async function referResource(recordId: string, recipientOrganizationId: string, note: string) {
    const { referral } = await createResourceReferral(recordId, recipientOrganizationId, note);
    setResourceWorkflow(undefined);
    setResourceNotice(`Referral ${referral.id} created. Track it in Menu → Referrals Management.`);
  }

  function handleResourceNavigationAction(action: ResourceNavigationAction) {
    const record = resourceNavigationRecord;
    if (action === "offer") { setResourceNavigationOpen(false); setResourceWorkflow({ mode: "offer" }); return; }
    if (action === "open-referrals-management") {
      setResourceNavigationOpen(false);
    ...
    setResourceNavigationOpen(false);
    if (action === "edit") setResourceWorkflow({ mode: "edit", recordId: record.id });
    if (action === "share") setResourceWorkflow({ mode: "share", recordId: record.id });
    if (action === "save-archive") setResourceWorkflow({ mode: "save-archive", recordId: record.id });
    if (action === "request") setResourceWorkflow({ mode: "request", recordId: record.id });
    if (action === "save-follow") setResourceWorkflow({ mode: "save-follow", recordId: record.id });
    if (action === "referral") setResourceWorkflow({ mode: "referral", recordId: record.id });
    if (action === "view") openDetail(record.id);
  }

  function createInsight(record: ExchangeRecord) {
    const next = record.ownedByViewer
      ? { ...record, organization: organizationName, metadata: record.metadata.filter((item) => item.toLowerCase() !== "owned by you") }
      : record;
    setRecordsState((current) => [next, ...current]);
  ...
  function updateInsight(record: ExchangeRecord) {
    const next = record.ownedByViewer
      ? { ...record, organization: organizationName, metadata: record.metadata.filter((item) => item.toLowerCase() !== "owned by you") }
      : record;
    setRecordsState((current) => current.map((item) => item.id === next.id ? next : item));
  ...
  async function handleAction(action: LensAction, record?: ExchangeRecord) {
    ...
    if (lens === "resources" && record) {
      if (action.id === "edit-resource") { setResourceWorkflow({ mode: "edit", recordId: record.id }); return; }
      if (action.id === "request-resource") { setResourceWorkflow({ mode: "request", recordId: record.id }); return; }
      if (action.id === "archive-resource") { setResourceWorkflow({ mode: "archive", recordId: record.id }); return; }
      if (action.id === "share") { setResourceWorkflow({ mode: "share", recordId: record.id }); return; }
      if (action.id === "refer") { setResourceWorkflow({ mode: "referral", recordId: record.id }); return; }
    }
    ...
    if (lens === "capabilities" && record && isCapabilityWorkflowMode(action.id)) {
      setCapabilityWorkflow({ mode: action.id, recordId: record.id });
      return;
    }
  ...
  function selectGeography(geography?: MapGeographyOption) {
    if (!geography) { resetMapView(); return; }
  ...
  const organizationAnchor = resolvedViewerContext.organization;
  const hasVisibleOwnMarker = visibleRecords.some((record) => record.ownedByViewer && record.location);
  const organizationAnchorRecord: ExchangeRecord | undefined = organizationAnchor?.location && !hasVisibleOwnMarker
    ? {
        id: SELF_ORGANIZATION_ANCHOR_ID,
        type: typeByLens[lens],
        title: organizationName,
        organization: organizationName,
        summary: "Signed-in organization map anchor.",
        geography: mapView.geography.label,
        metadata: [],
        location: organizationAnchor.location,
        ownedByViewer: true,
      }
    : undefined;
  const mapRecords = organizationAnchorRecord ? [...visibleRecords, organizationAnchorRecord] : visibleRecords;
  const drawerActiveActionIds = activeLensControlIds(actions);
  const detailActions = detailRecord ? definition.recordActions(detailRecord, resolvedViewerContext) : [];
  ...
  return (
    <main className="exchange-shell">
      ...
      <ResultsDrawer state={drawer} onStateChange={setDrawer} lens={lens} lensLabel={definition.label} records={visibleRecords} totalAvailableCount={viewportRecords.length} selectedRecordId={selectedRecordId} actions={actions} activeActionIds={drawerActiveActionIds} getRecordActions={(record) => definition.recordActions(record, resolvedViewerContext)} onAction={(action, record) => { void handleAction(action, record); }} emptyMessage={definition.emptyMessage} resultContext={`${mapped} mapped · ${offMap} off-map${mapView.queriedBounds ? " · map area applied" : ""}`} query={drawerQuery} onQueryChange={(next) => setDrawerQueries((current) => ({ ...current, [lens]: next }))} onSelect={selectRecord} onOpen={openDetail} onToggleSave={handleCardSave} onOpenWorkflowHierarchy={lens === "resources" ? () => setResourceNavigationOpen(true) : undefined} />

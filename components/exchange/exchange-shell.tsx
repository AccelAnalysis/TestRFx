...
export function ExchangeShell({ initialLens = "rfx", initialRecordId }: { initialLens?: ExchangeLens; initialRecordId?: string }) {
  ...
  const [recordsState, setRecordsState] = useState<ExchangeRecord[]>(exchangeSeed);
  ...
  const workflowRecord = resourceWorkflow && "recordId" in resourceWorkflow ? allRecords.find((record) => record.id === resourceWorkflow.recordId) : undefined;
  ...
  function changeLens(next: ExchangeLens) { const prior = searchByLens[next]; const carried = prior.query.trim() ? prior : { ...prior, query: searchState.query }; setLens(next); setSearchByLens((current) => ({ ...current, [next]: carried })); setSelectedRecordId(undefined); setDetailRecordId(undefined); setResourceWorkflow(undefined); setActionNotice(""); setUrl(next, undefined, "replace", carried); }
  ...
  return <main className="exchange-shell">
    <PersistentMap lens={lens} records={records.filter((record) => record.resource?.status !== "archived")} selectedRecordId={selectedRecordId} drawerState={drawer} view={mapView} viewerLocation={viewerLocation} onViewChange={(next) => { setMapView(next); setViewportDirty(true); }} onSelect={selectRecord} />
    ...
    {resourceWorkflow ? <ResourceWorkflowSurface workflow={resourceWorkflow} record={workflowRecord} onClose={() => setResourceWorkflow(undefined)} onCreate={createResource} onUpdate={updateResource} onRequest={requestResource} onArchive={archiveResource} /> : null}

...
export function ExchangeShell({ initialLens = "rfx", initialRecordId, viewerContext }: { initialLens?: ExchangeLens; initialRecordId?: string; viewerContext?: Partial<ExchangeViewerContext> }) {
  ...
  const intelligenceNavigationRecord = allRecords.find((record) => record.id === (detailRecordId ?? selectedRecordId) && record.type === "intelligence");
  ...
  useEffect(() => {
    let active = true;
    void listResourcesFromService().then(({ records: live }) => {
      if (!active) return; replaceDomainRecords("resource", live); syncSaved(live);
    }).catch((error) => { if (active && !pagesPreview()) { replaceDomainRecords("resource", []); setResourceNotice(error instanceof Error ? error.message : "Resources runtime is unavailable."); } });
    return () => { active = false; };
  }, []);
  ...
  async function requestResource(recordId: string, request: ResourceRequestDraft) { await requestResourceThroughService(recordId, request); const record = allRecords.find((item) => item.id === recordId); setResourceWorkflow(undefined); setResourceNotice(`Request sent for ${record?.title ?? "Resource"}${request.neededBy ? ` · needed ${request.neededBy}` : ""}.`); }
  ...
  const mapRecords = organizationAnchorRecord ? [...visibleRecords, organizationAnchorRecord] : visibleRecords; const drawerActiveActionIds = activeLensControlIds(actions); const detailActions = detailRecord ? definition.recordActions(detailRecord, resolvedViewerContext) : [];
  ...
  return <main className="exchange-shell">
    ...
    <ResultsDrawer state={drawer} onStateChange={setDrawer} lens={lens} lensLabel={definition.label} records={visibleRecords} totalAvailableCount={viewportRecords.length} selectedRecordId={selectedRecordId} actions={actions} activeActionIds={drawerActiveActionIds} getRecordActions={(record) => definition.recordActions(record, resolvedViewerContext)} onAction={(action, record) => { void handleAction(action, record); }} emptyMessage={definition.emptyMessage} resultContext={`${mapped} mapped · ${offMap} off-map${mapView.queriedBounds ? " · map area applied" : ""}`} query={drawerQuery} onQueryChange={(next) => setDrawerQueries((current) => ({ ...current, [lens]: next }))} onSelect={selectRecord} onOpen={openDetail} onToggleSave={handleCardSave} onOpenWorkflowHierarchy={lens === "resources" ? () => setResourceNavigationOpen(true) : lens === "intelligence" ? () => setIntelligenceNavigationNodeId("intelligence") : undefined} />

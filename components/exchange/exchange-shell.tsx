...
import { createDefaultMapView, mapBoundsForCamera } from "@/lib/exchange/map-model";
import { resourceMetadata, type ResourceDraft, type ResourceRequestDraft } from "@/lib/exchange/resources";
import { activeFilterCount, defaultSearchState, emptySearchResponse, searchExchangeRecords, searchStateFromParams, searchStateToParams, typeByLens } from "@/lib/exchange/search";
import { createSavedSearch, deleteSavedSearch, fetchExchangeSearch, fetchSearchLibrary, recordRecentSearch, SearchClientError, updateSavedSearch } from "@/lib/exchange/search-client";
...
const recentStorageKey = "rfxchange:preview:recent-searches";
const savedStorageKey = "rfxchange:preview:saved-searches";
const previewMode = process.env.NEXT_PUBLIC_RFXCHANGE_PREVIEW === "1";
...
function mergeRecords(...sets: ExchangeRecord[][]) {
  const merged = new Map<string, ExchangeRecord>();
  for (const set of sets) for (const record of set) merged.set(record.id, record);
...
export function ExchangeShell({ initialLens = "rfx", initialRecordId }: { initialLens?: ExchangeLens; initialRecordId?: string }) {
  ...
  const [remoteSearchByLens, setRemoteSearchByLens] = useState<Partial<Record<ExchangeLens, ExchangeSearchResponse>>>({});
  const [searchStatus, setSearchStatus] = useState<DrawerResultStatus>(previewMode ? "ready" : "loading");
  ...
  const previewSearchResponse = useMemo(() => searchExchangeRecords(allRecords, lens, searchState), [allRecords, lens, searchState]);
  const searchResponse = previewMode ? previewSearchResponse : remoteSearchByLens[lens] ?? emptySearchResponse(lens, searchState);
  const searchRecords = useMemo(() => searchResponse.results.map((result) => result.record), [searchResponse]);
  const recordCatalog = useMemo(() => mergeRecords(allRecords, searchRecords), [allRecords, searchRecords]);
  ...
  const lensRecords = useMemo(() => (previewMode ? allRecords : searchRecords).filter((record) => record.type === typeByLens[lens]), [allRecords, searchRecords, lens]);
  const selectedRecord = recordCatalog.find((record) => record.id === selectedRecordId);
  const detailRecord = recordCatalog.find((record) => record.id === detailRecordId);
  ...
  const resourceWorkflowRecord = resourceWorkflow && "recordId" in resourceWorkflow ? recordCatalog.find((record) => record.id === resourceWorkflow.recordId) : undefined;
  const intelligenceWorkflowRecord = intelligenceWorkflow?.recordId ? recordCatalog.find((record) => record.id === intelligenceWorkflow.recordId) : undefined;
  ...
  const currentMapBounds = useMemo(() => mapBoundsForCamera(mapView.camera), [mapView.camera]);

  async function refreshSearchLibrary(targetLens = lens) {
    if (previewMode) return;
    try {
      const library = await fetchSearchLibrary(targetLens);
      setRecentSearches((current) => [...current.filter((item) => item.lens !== targetLens), ...library.recent]);
      setSavedSearches((current) => [...current.filter((item) => item.lens !== targetLens), ...library.saved]);
      setLibraryNotice(undefined);
    } catch (error) {
      if (error instanceof SearchClientError && error.status === 401) setLibraryNotice("Sign in through the production identity service to persist recent and saved searches.");
      else if (error instanceof SearchClientError) setLibraryNotice(error.message);
      else setLibraryNotice("Search library is unavailable.");
    }
  }
  ...
  useEffect(() => {
    if (previewMode) { setSearchStatus("ready"); return; }
    const controller = new AbortController();
    const timeout = window.setTimeout(async () => {
      setSearchStatus(remoteSearchByLens[lens] ? "refreshing" : "loading");
      try {
        const response = await fetchExchangeSearch(lens, searchState, { limit: 30, signal: controller.signal });
        setRemoteSearchByLens((current) => ({ ...current, [lens]: response }));
        setSearchStatus("ready");
      } catch (error) {
        if (controller.signal.aborted) return;
        setSearchStatus(typeof navigator !== "undefined" && !navigator.onLine ? "offline" : "error");
        if (error instanceof SearchClientError) setActionNotice(error.message);
      }
    }, 180);
    return () => { controller.abort(); window.clearTimeout(timeout); };
  }, [lens, searchState, retryKey]);

  useEffect(() => { void refreshSearchLibrary(lens); }, [lens]);
  ...
  function persistPreviewRecent(next: RecentSearch[]) { setRecentSearches(next); try { localStorage.setItem(recentStorageKey, JSON.stringify(next)); } catch {} }
  function persistPreviewSaved(next: SavedSearch[]) { setSavedSearches(next); try { localStorage.setItem(savedStorageKey, JSON.stringify(next)); } catch {} }
  function setUrl(nextLens: ExchangeLens, recordId?: string, mode: "push" | "replace" = "replace", state = searchByLens[nextLens]) {
    ...
    const next = withBasePath(params ? `${path}?${params}` : path);
  ...
  function commitSearch(next: ExchangeSearchState) {
    ...
    if (previewMode) {
      ...
      const recent: RecentSearch = { id: `${lens}-${Date.now()}`, lens, state: next, createdAt: now };
      persistPreviewRecent([recent, ...recentSearches.filter((item) => item.lens !== lens || JSON.stringify(item.state) !== JSON.stringify(next))].slice(0, 12));
      return;
    }
    void recordRecentSearch(lens, next, searchResponse.total).then(() => refreshSearchLibrary(lens)).catch((error) => {
      if (error instanceof SearchClientError && error.status === 401) setLibraryNotice("Sign in through the production identity service to persist recent and saved searches.");
    });
  }

  async function saveCurrentSearch(name?: string, sourceState = searchState) {
    if (!sourceState.query.trim() && activeFilterCount(sourceState) === 0) return;
    const descriptor = sourceState.query.trim() || sourceState.filters.geography.trim() || "Discovery";
    const resolvedName = name?.trim() || `${definition.label}: ${descriptor}`;
    if (previewMode) {
      const now = new Date().toISOString();
      const saved: SavedSearch = { id: `${lens}-${Date.now()}`, name: resolvedName, lens, state: sourceState, alertEnabled: false, createdAt: now, updatedAt: now };
      persistPreviewSaved([saved, ...savedSearches.filter((item) => item.lens !== lens || JSON.stringify(item.state) !== JSON.stringify(sourceState))].slice(0, 20));
      return;
    }
    try {
      const saved = await createSavedSearch({ name: resolvedName, lens, state: sourceState });
      setSavedSearches((current) => [saved, ...current.filter((item) => item.id !== saved.id)]);
      setLibraryNotice(undefined);
    } catch (error) {
      setLibraryNotice(error instanceof SearchClientError ? error.message : "Unable to save this search.");
    }
  }

  async function updateExistingSavedSearch(id: string, patch: { name?: string; state?: ExchangeSearchState; alertEnabled?: boolean }) {
    if (previewMode) {
      const now = new Date().toISOString();
      const next = savedSearches.map((item) => item.id === id ? { ...item, ...patch, updatedAt: now } : item);
      persistPreviewSaved(next);
      return;
    }
    try {
      ...
      setLibraryNotice(error instanceof SearchClientError ? error.message : "Unable to update this saved search.");
    }
  }

  async function removeSavedSearch(id: string) {
    if (previewMode) { persistPreviewSaved(savedSearches.filter((item) => item.id !== id)); return; }
    try {
      await deleteSavedSearch(id);
      setSavedSearches((current) => current.filter((item) => item.id !== id));
      setLibraryNotice(undefined);
    } catch (error) {
      setLibraryNotice(error instanceof SearchClientError ? error.message : "Unable to delete this saved search.");
    }
  }
  ...
  function changeLens(next: ExchangeLens) {
    ...
    setLens(next); setSearchByLens((current) => ({ ...current, [next]: carried }));
    setSelectedRecordId(undefined); setDetailRecordId(undefined); setResourceWorkflow(undefined); setIntelligenceWorkflow(undefined); setCapabilityWorkflow(undefined); setActionNotice(""); setUrl(next, undefined, "replace", carried);
  }
  ...
  function actionIsActive(record: ExchangeRecord | undefined, action: LensAction) { if (!record || !action.toggle) return false; if (action.toggle === "save") return Boolean(record.saved) || savedRecordIds.has(record.id); if (lens === "capabilities" && action.id === "follow") return savedRecordIds.has(record.id); return actionState[actionKey(record, action)] ?? false; }
  ...
  function requestResource(recordId: string, request: ResourceRequestDraft) { const record = recordCatalog.find((item) => item.id === recordId); setResourceWorkflow(undefined); setResourceNotice(`Request created for ${record?.title ?? "resource"}${request.neededBy ? ` · needed ${request.neededBy}` : ""}. Provider notification and fulfillment remain shared-service integrations.`); }
  ...
  function searchCurrentMapArea() {
    const bounds = mapBoundsForCamera(mapView.camera);
    const next: ExchangeSearchState = { ...searchState, filters: { ...searchState.filters, geographyMode: "viewport", bounds, center: undefined, radiusMiles: undefined } };
    setMapView((current) => ({ ...current, queriedBounds: bounds }));
    setViewportDirty(false);
    commitSearch(next);
  }

  async function loadMoreResults() {
    if (previewMode || loadingMore || !searchResponse.hasMore || !searchResponse.nextCursor) return;
    setLoadingMore(true);
    try {
      const nextPage = await fetchExchangeSearch(lens, searchState, { cursor: searchResponse.nextCursor, limit: 30 });
      setRemoteSearchByLens((current) => {
        const previous = current[lens] ?? emptySearchResponse(lens, searchState);
        const byId = new Map(previous.results.map((result) => [result.record.id, result]));
        for (const result of nextPage.results) byId.set(result.record.id, result);
        return { ...current, [lens]: { ...nextPage, results: [...byId.values()] } };
      });
    } catch (error) {
      setActionNotice(error instanceof SearchClientError ? error.message : "Unable to load more results.");
    } finally {
  ...
  const serviceMode = previewMode ? "preview" : searchStatus === "error" || searchStatus === "offline" ? "unavailable" : "live";
  ...
  return <main className="exchange-shell">
    ...
    <SearchControls lens={lens} state={searchState} placeholder={definition.searchPlaceholder} lensLabel={definition.label} suggestions={searchResponse.suggestions} recentSearches={lensRecent} savedSearches={lensSaved} searchStatus={searchStatus} serviceMode={serviceMode} libraryNotice={libraryNotice} mapCenter={mapView.camera.center} mapBounds={currentMapBounds} exchangeGeography={mapView.geography.label} onStateChange={updateSearchState} onCommit={commitSearch} onRunState={(next) => { updateSearchState(next); commitSearch(next); }} onSave={saveCurrentSearch} onUpdateSaved={updateExistingSavedSearch} onDeleteSaved={removeSavedSearch} />
    <FloatingControls lens={lens} records={lensRecords.filter((record) => record.resource?.status !== "archived")} search={searchState.query} filters={floatingFilters} onFiltersChange={(next) => setFiltersByLens((current) => ({ ...current, [lens]: next }))} mapDisplayMode={mapView.camera.mode} onMapDisplayModeChange={(mode: MapDisplayMode) => setMapView((current) => ({ ...current, camera: { ...current.camera, mode, pitch: mode === "3d" ? 42 : 0 } }))} geolocationStatus={geolocationStatus} onLocate={locateViewer} onResetView={resetMapView} searchAreaAvailable={viewportDirty} onSearchArea={searchCurrentMapArea} />
    <ResultsDrawer state={drawer} onStateChange={setDrawer} lens={lens} lensLabel={definition.label} records={visibleRecords} totalAvailableCount={searchResponse.total} selectedRecordId={selectedRecordId} actions={actions} activeActionIds={drawerActiveActionIds} onAction={(action) => { void handleAction(action, actionRecord); }} emptyMessage={definition.emptyMessage} resultContext={`${searchResponse.total} results · ${searchResponse.mapped} mapped · ${searchResponse.offMap} off-map`} query={drawerQuery} onQueryChange={(next) => setDrawerQueries((current) => ({ ...current, [lens]: next }))} onSelect={selectRecord} onOpen={openDetail} onToggleSave={toggleSaved} resultStatus={searchStatus} hasMore={searchResponse.hasMore} loadingMore={loadingMore} onLoadMore={() => { void loadMoreResults(); }} onRetry={() => setRetryKey((value) => value + 1)} />
    ...
    {intelligenceWorkflow ? <IntelligenceWorkflowSurface workflow={intelligenceWorkflow.mode} record={intelligenceWorkflowRecord} records={recordCatalog} onClose={() => setIntelligenceWorkflow(undefined)} onCreate={createInsight} onUpdate={updateInsight} onAddNote={addIntelligenceNote} /> : null}

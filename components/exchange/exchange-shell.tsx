...
import { getIntelligenceNavigationPath, resultNodeForAction, type IntelligenceListResponse, type IntelligenceWorkflow } from "@/lib/exchange/intelligence";
import { IntelligenceServiceError, listIntelligenceRecords, updateIntelligenceTracking } from "@/lib/exchange/intelligence-client";
...
const initialRecords = [...exchangeSeed.filter((record) => record.type !== "intelligence" && record.type !== "capability"), ...capabilityExchangeRecords];
...
export function ExchangeShell({ initialLens = "rfx", initialRecordId }: { initialLens?: ExchangeLens; initialRecordId?: string }) {
  ...
  const [intelligenceRecords, setIntelligenceRecords] = useState<ExchangeRecord[]>([]);
  const [intelligenceSummary, setIntelligenceSummary] = useState<Omit<IntelligenceListResponse, "records">>(emptyIntelligenceSummary);
  const [intelligenceStatus, setIntelligenceStatus] = useState<DrawerResultStatus>(initialLens === "intelligence" ? "loading" : "ready");
  const [intelligenceLoadingMore, setIntelligenceLoadingMore] = useState(false);
  ...
  const [intelligenceWorkflow, setIntelligenceWorkflow] = useState<IntelligenceFlow>(); const [intelligenceNavStack, setIntelligenceNavStack] = useState<IntelligenceNavEntry[]>([]);
  ...
  const localRecords = useMemo(() => recordsState.map((record) => ({ ...record, saved: savedRecordIds.has(record.id) })), [recordsState, savedRecordIds]);
  const allRecords = useMemo(() => [...localRecords, ...intelligenceRecords], [localRecords, intelligenceRecords]);
  ...
  const searchRecords = useMemo(() => lens === "intelligence" ? intelligenceRecords : searchResponse.results.map((result) => result.record), [lens, intelligenceRecords, searchResponse]);
  ...
  const intelligenceWorkflowRecord = intelligenceWorkflow?.recordId ? intelligenceRecords.find((record) => record.id === intelligenceWorkflow.recordId) : undefined;
  const intelligenceNav = intelligenceNavStack[intelligenceNavStack.length - 1];
  const intelligenceNavRecord = intelligenceNav?.recordId ? intelligenceRecords.find((record) => record.id === intelligenceNav.recordId) : undefined;
  ...
  useEffect(() => { if (selectedRecordId && !records.some((record) => record.id === selectedRecordId) && lens !== "intelligence") setSelectedRecordId(undefined); }, [records, selectedRecordId, lens]);
  ...
  function changeLens(next: ExchangeLens) { const prior = searchByLens[next]; const carried = prior.query.trim() ? prior : { ...prior, query: searchState.query }; setLens(next); setSearchByLens((current) => ({ ...current, [next]: carried })); setSelectedRecordId(undefined); setDetailRecordId(undefined); setResourceWorkflow(undefined); setIntelligenceWorkflow(undefined); setIntelligenceNavStack([]); setCapabilityWorkflow(undefined); setActionNotice(""); setUrl(next, undefined, "replace", carried); }
  ...
  function replaceIntelligenceRecord(record: ExchangeRecord) { setIntelligenceRecords((current) => { const exists = current.some((item) => item.id === record.id); return exists ? current.map((item) => item.id === record.id ? record : item) : [record, ...current]; }); setSelectedRecordId(record.id); }
  function setIntelligenceNavigation(nodeId: string, recordId?: string) { setIntelligenceNavStack(getIntelligenceNavigationPath(nodeId).map((node) => ({ nodeId: node.id, recordId }))); }
  function navigateIntelligence(nodeId: string) { setIntelligenceNavStack((current) => { const existing = current.findIndex((item) => item.nodeId === nodeId); if (existing >= 0) return current.slice(0, existing + 1); return [...current, { nodeId, recordId: current[current.length - 1]?.recordId }]; }); }
  function backIntelligence() { setIntelligenceNavStack((current) => current.length > 1 ? current.slice(0, -1) : []); }
  ...
  return <main className="exchange-shell">
    ...
    <ResultsDrawer state={drawer} onStateChange={setDrawer} lens={lens} lensLabel={definition.label} records={visibleRecords} totalAvailableCount={drawerTotal} selectedRecordId={selectedRecordId} actions={actions} activeActionIds={drawerActiveActionIds} onAction={(action) => { void handleAction(action, actionRecord); }} emptyMessage={definition.emptyMessage} resultContext={`${mapped} mapped · ${offMap} off-map`} query={drawerQuery} onQueryChange={(next) => setDrawerQueries((current) => ({ ...current, [lens]: next }))} onSelect={selectRecord} onOpen={openDetail} onToggleSave={(id) => { void toggleCardRelationship(id); }} resultStatus={lens === "intelligence" ? intelligenceStatus : "ready"} hasMore={lens === "intelligence" && intelligenceSummary.nextOffset != null} loadingMore={lens === "intelligence" && intelligenceLoadingMore} onLoadMore={lens === "intelligence" ? () => { void loadMoreIntelligence(); } : undefined} onRetry={lens === "intelligence" ? () => setIntelligenceRefreshKey((value) => value + 1) : undefined} />
    ...
    {intelligenceWorkflow ? <IntelligenceWorkflowSurface workflow={intelligenceWorkflow.mode} record={intelligenceWorkflowRecord} records={intelligenceRecords} onClose={() => setIntelligenceWorkflow(undefined)} onCompleted={completeIntelligenceWorkflow} /> : null}
    {intelligenceNav ? <IntelligenceNavigationSurface nodeId={intelligenceNav.nodeId} record={intelligenceNavRecord} refreshKey={intelligenceRefreshKey} onNavigate={navigateIntelligence} onBack={backIntelligence} onClose={() => setIntelligenceNavStack([])} onOpenEdit={() => { if (intelligenceNavRecord) setIntelligenceWorkflow({ mode: "edit", recordId: intelligenceNavRecord.id }); }} onOpenMatch={openMatchedRecord} /> : null}

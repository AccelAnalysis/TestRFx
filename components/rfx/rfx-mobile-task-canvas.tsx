...
export interface RfxTaskCanvasProps {
...
const templates = [
  { id: "quick-service", label: "Quick service request", description: "A short request for a clearly defined service.", need: "We need a qualified provider to deliver a defined service by a specified date.", type: "Service Request", mode: "quick" as const },
  { id: "competitive-rfq", label: "Competitive quote", description: "A specification-led request where price and delivery matter most.", need: "We need comparable quotes for a defined product or service specification.", type: "RFQ", mode: "guided" as const },
  { id: "sources-sought", label: "Sources sought", description: "Identify capable organizations before designing a formal opportunity.", need: "We want to identify organizations with the capability and capacity to perform this work.", type: "Sources Sought", mode: "guided" as const },
];
...
function MobileField({ field, value, onChange, onDictate, dictating }: {
  ...
  if (field.type === "select") {
    return <fieldset className={styles.choiceField}><legend>{field.label}{field.required ? " *" : ""}</legend>{field.help ? <p>{field.help}</p> : null}<div className={styles.choiceGrid}>{(field.options ?? []).map((option) => { const active = valueAsString(value) === option; return <button key={option} type="button" className={active ? styles.choiceActive : styles.choiceButton} aria-pressed={active} onClick={() => onChange(option)}><span>{option}</span>{active ? <Check aria-hidden size={18} /> : <Circle aria-hidden size={16} />}</button>; })}</div></fieldset>;
  }
  if (field.type === "boolean") {
    return <button type="button" className={Boolean(value) ? styles.toggleActive : styles.toggle} aria-pressed={Boolean(value)} onClick={() => onChange(!Boolean(value))}><span><strong>{field.label}</strong>{field.help ? <small>{field.help}</small> : null}</span><span className={styles.toggleTrack}><span /></span></button>;
  }
  if (field.type === "number") {
    const numeric = Number(value || 0);
    return <label className={styles.field}><span>{field.label}{field.required ? " *" : ""}</span><div className={styles.stepper}><button type="button" aria-label={`Decrease ${field.label}`} onClick={() => onChange(Math.max(0, numeric - 1))}><Minus size={18} /></button><input type="number" inputMode="decimal" value={valueAsString(value)} placeholder={field.placeholder} onChange={(event) => onChange(event.target.value ? Number(event.target.value) : 0)} /><button type="button" aria-label={`Increase ${field.label}`} onClick={() => onChange(numeric + 1)}><Plus size={18} /></button></div>{field.help ? <small>{field.help}</small> : null}</label>;
  }
  const long = field.type === "textarea";
  return <label className={styles.field}><span>{field.label}{field.required ? " *" : ""}</span><div className={styles.inputShell}>{long ? <textarea value={valueAsString(value)} placeholder={field.placeholder} onChange={(event) => onChange(event.target.value)} /> : <input type={field.type === "date" ? "date" : "text"} value={valueAsString(value)} placeholder={field.placeholder} onChange={(event) => onChange(event.target.value)} />}{field.type !== "date" ? <button type="button" className={dictating ? styles.micActive : styles.mic} onClick={onDictate} aria-label={dictating ? "Stop dictation" : `Dictate ${field.label}`}><Mic size={19} /></button> : null}</div>{field.help ? <small>{field.help}</small> : null}</label>;
}

function PreflightPanel({ result, onOpen }: { result: RfxPreflightResult; onOpen: (path: string[]) => void }) {
  return <section className={styles.preflight} aria-label="Readiness preflight"><div className={styles.preflightHero}><div className={styles.progressRing} style={{ "--progress": `${result.percent}%` } as CSSProperties}><strong>{result.percent}%</strong><span>ready</span></div><div><p className={styles.eyebrow}>{result.ready ? "Ready for final review" : "Work remains"}</p><h3>{result.complete} of {result.total} checks complete</h3><p>{result.ready ? "No modeled blockers remain." : `${result.blockers.length} blocker${result.blockers.length === 1 ? "" : "s"} must be resolved first.`}</p></div></div><div className={styles.preflightList}>{result.items.map((item) => <button key={item.id} type="button" disabled={!item.path} onClick={() => item.path && onOpen(item.path)} className={styles.preflightItem}>{item.state === "complete" ? <CheckCircle2 className={styles.successIcon} size={22} /> : item.state === "blocker" ? <XCircle className={styles.blockerIcon} size={22} /> : <AlertTriangle className={styles.warningIcon} size={22} />}<span><strong>{item.label}</strong><small>{item.message}</small></span>{item.path ? <ChevronRight size={18} /> : null}</button>)}</div></section>;
}

type TransactionResult = { receiptId?: string; committedAt?: string; externalReference?: string; state?: string };

export function RfxMobileTaskCanvas(props: RfxTaskCanvasProps) {
  ...
  useEffect(() => {
    ...
    setLoadError("");
    void loadRfxWorkspace(props.record.id, props.entry)
      .then((loaded) => { if (active) setEnvelope(loaded); })
      .catch((error) => { if (active) setLoadError(error instanceof Error ? error.message : "The RFx workspace could not be opened."); });
    return () => { active = false; };
  }, [props.record.id, props.entry]);

  if (!envelope) return <div className={styles.backdrop}><section className={styles.canvas}><div className={styles.loading}>{loadError ? <WifiOff size={30} /> : <span />}<strong>{loadError ? "RFx workspace unavailable" : "Opening RFx workspace"}</strong><small>{loadError || "Your saved work is being restored."}</small>{loadError ? <button type="button" className={styles.primaryLarge} onClick={props.onClose}>Return to Exchange</button> : null}</div></section></div>;
  return <LoadedTaskCanvas key={`${props.record.id}:${props.entry}`} {...props} initialEnvelope={envelope} />;
}

function LoadedTaskCanvas({ record, entry, onClose, onOpenDetail, onToggleWatch, onLensHandoff, onOpenMenu, initialEnvelope }: RfxTaskCanvasProps & { initialEnvelope: RfxWorkspaceEnvelope }) {
  ...
  const root = useMemo(() => findWorkflowNode(tree, [rootId]) ?? tree[0], [tree, rootId]);
  const [envelope, setEnvelope] = useState<RfxWorkspaceEnvelope>(initialEnvelope);
  const [saveState, setSaveState] = useState<SaveState>(initialEnvelope.persistence === "postgres" ? "saved" : "offline");
  ...
  const node = findWorkflowNode(tree, workspace.activePath) ?? root;
  const currentFields = node.fields ?? [];
  const currentField = currentFields[Math.min(fieldIndex, Math.max(0, currentFields.length - 1))];
  const currentAttachments = workspace.items.filter((item) => item.nodeId === node.id && attachmentId(item));
  const currentItems = workspace.items.filter((item) => item.nodeId === node.id && !attachmentId(item));

  useEffect(() => () => { if (saveTimer.current) clearTimeout(saveTimer.current); recognition.current?.stop(); }, []);
  ...
  async function persist(next: RfxWorkspace, success?: string) {
    ...
    try {
      ...
      setEnvelope(saved);
      setSaveState(saved.persistence === "postgres" ? "saved" : "offline");
  ...
  function queuePersist(next: RfxWorkspace) {
    setEnvelope((current) => ({ ...current, workspace: next }));
    setSaveState("saving");
    if (saveTimer.current) clearTimeout(saveTimer.current);
  ...
  async function completeCurrent() {
    ...
    if (node.id === "go-no-go") {
      ...
      next = setPursuitState(next, decision === "Pursue" ? "pursuing" : decision === "Watch" ? "watching" : decision === "Decline" ? "declined" : next.pursuitState ?? "assessing");
    }
    ...
    const visible = perspective === "issuer" && entry === "create-rfx" ? visibleCreationChapters(all, experienceMode(next)) : all;
    const nextPath = nextVisiblePath(visible) ?? nextIncompletePath(root, next);
  ...
  async function addListItem(event: FormEvent) {
    ...
    if (!nodeRequiredFieldsComplete(node, workspace)) { setMessage("Complete the required information before adding this item."); return; }
  ...
  async function addAttachments(event: ChangeEvent<HTMLInputElement>) {
    ...
    for (const file of Array.from(event.target.files)) {
      ...
      } catch { setMessage("This browser could not store the selected attachment on the device."); }
    }
  ...
  function applyTemplate(templateId: string) {
    ...
    const values: Record<string, RfxWorkspaceValue> = {
      ...
      "need.statement": template.need,
      "mobile.recommendedType": template.type,
      "need.rfxType": template.type,
  ...
  async function startCreation() {
    ...
    let next = setWorkspaceValues(workspace, {
      ...
      "need.rfxType": recommendation.type,
      "experience.mode": workspace.values["experience.mode"] ?? recommendation.mode,
    });
    next = completeWorkspaceNode(next, "need");
    await persist(next, "Need captured");
    const path = pathToNode(tree, "select-rfx-type");
  ...
  async function decidePursuit(decision: "Pursue" | "Watch" | "Decline") {
    ...
    openById(decision === "Decline" ? "go-no-go" : "assess-fit");
  }
  ...
  async function runMarketPreview() {
    ...
    try {
      ...
      setMarketPreview({ loading: false, potential: results.length, criteria: results.filter((result) => (result.record?.metadata?.length ?? 0) > 0).length, geography: results.filter((result) => Boolean(result.record?.geography)).length, ready: results.filter((result) => Boolean(result.record?.organization && result.record?.summary)).length });
    } catch {
  ...
  async function executeTransaction(action: "publish" | "submit-hosted" | "record-external", payload: Record<string, unknown> = {}) {
    const response = await fetch("/api/rfx/transactions", { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, recordId: record.id, workspaceVersion: workspace.version, ...payload }) });
  ...
  async function publishRfx() {
    ...
    try {
      ...
      let next = setRfxStatus(completeWorkspaceNode(setWorkspaceValues(workspace, { "publish.confirmation": "Publish RFx" }), "publish"), "open");
      ...
      pulse();
    } catch (error) { setMessage(error instanceof Error ? error.message : "RFx publication is unavailable."); }
  }
  ...
  async function submitHosted() {
    ...
    try {
      ...
      let next = setPursuitState(completeWorkspaceNode(workspace, "hosted-submission"), "submitted");
      ...
      pulse();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Hosted submission is unavailable."); }
  }
  ...
  async function confirmExternalSubmission() {
    ...
    try {
      ...
      let next = setPursuitState(completeWorkspaceNode(workspace, "external-submission"), "submitted");
      ...
      pulse();
    } catch (error) { setMessage(error instanceof Error ? error.message : "External submission status could not be recorded."); }
  }
  ...
  const saveLabel = saveState === "saving" ? "Saving" : saveState === "saved" ? "Saved" : saveState === "offline" ? "Saved on device" : "Save issue";

  function renderCreationStart() {
    return <div className={styles.focusTask}><div className={styles.focusHeading}><span className={styles.iconDisc}><Sparkles size={23} /></span><p className={styles.eyebrow}>Create an RFx</p><h3>What do you need?</h3><p>Start in your own words. RFxchange will organize the request without forcing you through a giant form.</p></div><label className={styles.heroInput}><span className="sr-only">Describe what you need</span><textarea value={needStatement} placeholder="For example: We need a firm to assess our cybersecurity controls and provide a remediation plan…" onChange={(event) => updateValue("mobile.needStatement", event.target.value)} /><div className={styles.captureRail}><button type="button" onClick={() => startDictation({ id: "mobile.needStatement", label: "Need", type: "textarea" })} className={dictatingFieldId === "mobile.needStatement" ? styles.captureActive : styles.capture}><Mic size={18} /><span>Speak</span></button><label className={styles.capture}><Camera size={18} /><span>Scan</span><input className="sr-only" type="file" accept="image/*" capture="environment" onChange={addAttachments} /></label><label className={styles.capture}><Upload size={18} /><span>Upload</span><input className="sr-only" type="file" accept="image/*,.pdf,.doc,.docx,.txt" multiple onChange={addAttachments} /></label></div></label><div className={styles.templateBlock}><div><strong>Start another way</strong><small>Use a governed starting point and customize it.</small></div><div className={styles.templateScroller}>{templates.map((template) => <button key={template.id} type="button" className={selectedTemplate === template.id ? styles.templateActive : styles.template} onClick={() => applyTemplate(template.id)}><FileText size={20} /><strong>{template.label}</strong><small>{template.description}</small></button>)}</div></div><button type="button" className={styles.primaryLarge} onClick={() => void startCreation()}>Continue<ChevronRight size={19} /></button></div>;
  }

  function renderOpportunityDecision() {
    return <div className={styles.decisionHome}><div className={styles.opportunityHero}><div className={styles.opportunityTop}><span>{detail?.rfxType ?? "RFx"}</span><span className={styles.due}><Clock size={15} />{dueLabel(detail?.closesAt)}</span></div><h3>{record.title}</h3><p>{record.organization}</p><div className={styles.opportunityMeta}><span><MapPin size={15} />{detail?.performanceGeography ?? record.geography}</span>{detail?.estimatedValue ? <span>{detail.estimatedValue}</span> : null}<span>{effort.label} effort</span></div></div><section className={styles.whyCard}><p className={styles.eyebrow}>Why you are seeing this</p><h4>{breakdown.matched} of {breakdown.total} requirements represented</h4><div className={styles.matchTiles}><div className={styles.matched}><strong>{breakdown.matched}</strong><span>Matched</span></div><div className={styles.confirm}><strong>{breakdown.confirm}</strong><span>Confirm</span></div><div className={styles.gap}><strong>{breakdown.gap}</strong><span>Gap</span></div></div><p>{detail?.match?.summary ?? "Review the structured requirements before making a pursuit decision."}</p><small>Discovery context only—not qualification, endorsement, eligibility, or an award prediction.</small></section><section className={styles.effortCard}><Target size={22} /><div><strong>{effort.label} response effort</strong><p>{effort.detail}</p></div></section><div className={styles.decisionButtons}><button type="button" className={styles.primaryLarge} onClick={() => void decidePursuit("Pursue")}>Pursue</button><button type="button" onClick={() => void decidePursuit("Watch")}>Watch</button><button type="button" onClick={() => void decidePursuit("Decline")}>Decline</button></div></div>;
  }

  function renderResponseHome() {
    const next = nextVisiblePath(chapters);
    return <div className={styles.responseHome}><section className={styles.readinessHero}><div className={styles.progressRingLarge} style={{ "--progress": `${submission.percent}%` } as CSSProperties}><strong>{submission.percent}%</strong><span>ready</span></div><div><p className={styles.eyebrow}>Your response</p><h3>{dueLabel(detail?.closesAt)}</h3><p>{submission.blockers.length ? `${submission.blockers.length} blocker${submission.blockers.length === 1 ? "" : "s"} before submission` : "No modeled blockers remain"}</p></div></section>{next ? <button type="button" className={styles.nextBest} onClick={() => void navigate(next)}><span><small>Next best action</small><strong>Continue where you left off</strong></span><ChevronRight size={22} /></button> : null}{submission.blockers.length ? <section className={styles.blockerSummary}><div><AlertTriangle size={20} /><strong>Needs attention</strong></div>{submission.blockers.slice(0, 3).map((item) => <button key={item.id} type="button" onClick={() => item.path && void navigate(item.path)}><span>{item.label}</span><ChevronRight size={17} /></button>)}</section> : null}<div className={styles.chapterList}>{chapters.map((chapter) => <button key={chapter.id} type="button" className={styles.chapterCard} onClick={() => void navigate(chapter.nextPath ?? chapter.path)}><div className={chapter.percent === 100 ? styles.chapterStatusComplete : styles.chapterStatus}>{chapter.percent === 100 ? <Check size={18} /> : <span>{chapter.percent}%</span>}</div><span><strong>{chapter.label}</strong><small>{chapter.complete} of {chapter.total} tasks complete</small></span><ChevronRight size={19} /></button>)}</div><button type="button" className={styles.submitShortcut} onClick={() => openById(detail?.externalSubmissionRequired ? "external-submission" : "hosted-submission")}><ShieldCheck size={20} /><span><strong>Review & submit</strong><small>Run the final package preflight.</small></span><ChevronRight size={19} /></button></div>;
  }

  function renderChapterHome() {
    const next = nextVisiblePath(chapters);
    return <div className={styles.chapterHome}><section className={styles.chapterHero}><div className={styles.progressRingLarge} style={{ "--progress": `${rootProgress.percent}%` } as CSSProperties}><strong>{rootProgress.percent}%</strong><span>complete</span></div><div><p className={styles.eyebrow}>{perspective === "issuer" ? "Issuer workspace" : "Response workspace"}</p><h3>{entry === "create-rfx" ? "Build your RFx" : node.label}</h3><p>{mode[0].toUpperCase() + mode.slice(1)} path · {rootProgress.complete} of {rootProgress.total} tasks complete</p></div></section>{entry === "create-rfx" && needStatement ? <section className={styles.needSummary}><Sparkles size={20} /><div><small>Your need</small><p>{needStatement}</p><span>{valueAsString(workspace.values["mobile.recommendedType"]) || recommendation.type} · {mode} path</span></div></section> : null}{next ? <button type="button" className={styles.nextBest} onClick={() => void navigate(next)}><span><small>Continue</small><strong>Pick up where you left off</strong></span><ChevronRight size={22} /></button> : null}<div className={styles.chapterList}>{chapters.map((chapter) => <button key={chapter.id} type="button" className={styles.chapterCard} onClick={() => void navigate(chapter.nextPath ?? chapter.path)}><div className={chapter.percent === 100 ? styles.chapterStatusComplete : styles.chapterStatus}>{chapter.percent === 100 ? <Check size={18} /> : <span>{chapter.percent}%</span>}</div><span><strong>{chapter.label}</strong><small>{chapter.description}</small></span><ChevronRight size={19} /></button>)}</div></div>;
  }

  function renderTypeRecommendation() {
    const field = firstField(node);
    const selected = valueAsString(workspace.values[field?.id ?? "mobile.recommendedType"]) || recommendation.type;
    return <div className={styles.focusTask}><div className={styles.focusHeading}><span className={styles.iconDisc}><Sparkles size={23} /></span><p className={styles.eyebrow}>Recommended request</p><h3>{recommendation.type}</h3><p>{recommendation.reason}</p></div><div className={styles.recommendationCard}><span><small>Recommended</small><strong>{recommendation.type}</strong><p>{recommendation.reason}</p></span><CheckCircle2 size={25} /></div><div className={styles.alternativeBlock}><strong>Choose another type</strong><div className={styles.choiceGrid}>{[recommendation.type, ...recommendation.alternatives].filter((value, index, values) => values.indexOf(value) === index).map((type) => <button key={type} type="button" className={selected === type ? styles.choiceActive : styles.choiceButton} onClick={() => { if (field) updateValue(field.id, type); updateValue("mobile.recommendedType", type); }}><span>{type}</span>{selected === type ? <Check size={18} /> : <Circle size={16} />}</button>)}</div></div><div className={styles.modeBlock}><strong>How much structure do you need?</strong><div className={styles.modeGrid}>{(["quick", "guided", "formal"] as RfxExperienceMode[]).map((candidate) => <button key={candidate} type="button" className={mode === candidate ? styles.modeActive : styles.mode} onClick={() => updateValue("experience.mode", candidate)}><strong>{candidate[0].toUpperCase() + candidate.slice(1)}</strong><small>{candidate === "quick" ? "Defined need, lighter path" : candidate === "guided" ? "Typical RFx with structured guidance" : "Approvals, evaluation, and governance"}</small></button>)}</div></div></div>;
  }
  ...
  function renderGenericTask() {
    if (node.children?.length) return <div className={styles.chapterList}>{node.children.map((child) => { const complete = isNodeComplete(child, workspace); return <button key={child.id} type="button" className={styles.chapterCard} onClick={() => void navigate([...workspace.activePath, child.id])}><div className={complete ? styles.chapterStatusComplete : styles.chapterStatus}>{complete ? <Check size={18} /> : <Circle size={16} />}</div><span><strong>{child.label}</strong><small>{child.description}</small></span><ChevronRight size={19} /></button>; })}</div>;
  ...
  return <div className={styles.backdrop} role="presentation"><section className={styles.canvas} role="dialog" aria-modal="true" aria-label={`${node.label} RFx task canvas`}><header className={styles.header}><button type="button" className={styles.iconButton} onClick={goBack} aria-label="Go back"><ArrowLeft size={22} /></button><div className={styles.headerCenter}><strong>{entry === "create-rfx" ? "Create RFx" : perspective === "issuer" ? "Manage RFx" : "RFx response"}</strong><span>{atRoot ? `${rootProgress.percent}% complete` : node.label}</span></div><div className={styles.headerActions}><span className={saveState === "error" ? styles.saveError : styles.saveStatus}>{saveState === "saved" ? <Cloud size={15} /> : saveState === "offline" ? <HardDrive size={15} /> : saveState === "error" ? <WifiOff size={15} /> : <Save size={15} />}{saveLabel}</span><button type="button" className={styles.iconButton} onClick={() => setContextPanel(contextPanel === "more" ? undefined : "more")} aria-label="More RFx actions"><MoreHorizontal size={22} /></button><button type="button" className={styles.iconButton} onClick={() => void saveAndExit()} aria-label="Save and close"><X size={22} /></button></div></header>{!atRoot ? <div className={styles.stageBar}><div style={{ width: `${rootProgress.percent}%` }} /></div> : null}{contextPanel === "more" ? <div className={styles.moreSheet}><button type="button" onClick={onOpenDetail}><Eye size={19} /><span>View RFx detail</span></button><button type="button" onClick={() => setContextPanel("match")}><Target size={19} /><span>Match context</span></button><button type="button" onClick={() => setContextPanel("refer")}><Users size={19} /><span>Refer organization</span></button><button type="button" onClick={onToggleWatch}>{record.saved ? <Check size={19} /> : <Save size={19} />}<span>{record.saved ? "Saved / watching" : "Save / watch"}</span></button></div> : null}{contextPanel === "match" ? <div className={styles.contextSheet}><div className={styles.sheetHeader}><strong>Match context</strong><button type="button" onClick={() => setContextPanel(undefined)}><X size={20} /></button></div><div className={styles.matchTiles}><div className={styles.matched}><strong>{breakdown.matched}</strong><span>Matched</span></div><div className={styles.confirm}><strong>{breakdown.confirm}</strong><span>Confirm</span></div><div className={styles.gap}><strong>{breakdown.gap}</strong><span>Gap</span></div></div><p>{detail?.match?.summary ?? "No structured match context is available."}</p><small>Discovery context only—not qualification or an award prediction.</small></div> : null}{contextPanel === "refer" ? <div className={styles.contextSheet}><div className={styles.sheetHeader}><strong>Refer from RFx context</strong><button type="button" onClick={() => setContextPanel(undefined)}><X size={20} /></button></div><MobileField field={rfxContextActionTree[2].fields![0]} value={workspace.values["contextReferral.organization"]} onChange={(value) => updateValue("contextReferral.organization", value)} onDictate={() => startDictation(rfxContextActionTree[2].fields![0])} dictating={dictatingFieldId === rfxContextActionTree[2].fields![0].id} /><MobileField field={rfxContextActionTree[2].fields![1]} value={workspace.values["contextReferral.note"]} onChange={(value) => updateValue("contextReferral.note", value)} onDictate={() => startDictation(rfxContextActionTree[2].fields![1])} dictating={dictatingFieldId === rfxContextActionTree[2].fields![1].id} /><button type="button" className={styles.primaryLarge} onClick={() => void createContextReferral()}>Create referral</button></div> : null}<main className={styles.body} key={node.id}>{!atRoot && !["hosted-submission", "external-submission"].includes(treatment) ? <div className={styles.taskHeading}><div><p className={styles.eyebrow}>{node.label}</p><h2>{node.label}</h2><p>{node.description}</p></div>{currentFields.length > 1 ? <span className={styles.taskCount}>{currentFieldNumber} / {currentFields.length}</span> : null}</div> : null}{taskContent}{currentAttachments.length ? <section className={styles.attachments}><strong>Device attachments</strong>{currentAttachments.map((item) => <article key={item.id}><Paperclip size={18} /><span><strong>{item.label}</strong><small>{item.note}</small></span><button type="button" onClick={() => void removeAttachment(item)} aria-label={`Remove ${item.label}`}><X size={17} /></button></article>)}</section> : null}{message ? <div className={styles.message} role="status" aria-live="polite">{message}</div> : null}</main>{node.id === "publish" && !completionMoment ? <footer className={styles.commitFooter}><button type="button" onClick={() => void saveAndExit()}>Save & exit</button><button type="button" className={styles.primary} disabled={!publication.ready} onClick={() => void publishRfx()}><Send size={18} />Publish RFx</button></footer> : null}{completionMoment === "published" ? <div className={styles.successOverlay}><span><Check size={34} /></span><p className={styles.eyebrow}>Published</p><h2>Your RFx is live</h2><p>Discovery, matching, and the response timeline are now active.</p><button type="button" className={styles.primaryLarge} onClick={onClose}>Return to Exchange</button></div> : null}{canShowTaskFooter ? <footer className={styles.footer}><button type="button" onClick={() => void saveAndExit()}>Save & exit</button>{node.children?.length ? <button type="button" className={styles.primary} onClick={() => { const next = node.children?.find((child) => !isNodeComplete(child, workspace)) ?? node.children?.[0]; if (next) void navigate([...workspace.activePath, next.id]); }}>Continue<ChevronRight size={18} /></button> : node.kind === "list" ? null : <button type="button" className={styles.primary} onClick={nextFieldOrComplete}>{primaryLabel}<ChevronRight size={18} /></button>}</footer> : null}</section></div>;
}

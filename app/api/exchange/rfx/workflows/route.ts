...
function errorResponse(error: unknown) {
  ...
  return NextResponse.json({ error: message }, { status: /required|not found|not allowed/i.test(message) ? 422 : 500 });
}

async function findRecord(client: import("pg").PoolClient, publicId: string) {
  const result = await client.query<RecordRow>(
    `SELECT er.id, er.public_id, er.title, er.summary, er.organization_id, o.name AS organization_name, rr.lifecycle_status
     FROM exchange_records er JOIN organizations o ON o.id=er.organization_id JOIN rfx_records rr ON rr.exchange_record_id=er.id
     WHERE er.public_id=$1 AND er.record_type='rfx'`, [publicId]);
...
export async function POST(request: NextRequest) {
  ...
  const actor = (() => { try { return resolveExchangeActor(request.headers); } catch (error) { throw error; } })();
    ...
    const result = await withExchangeTransaction(async (client) => {
      if (body.command === "draft" || body.command === "save" || body.command === "publish") {
        ...
        const created = await client.query<{ id: string }>(
          `INSERT INTO exchange_records (public_id, record_type, organization_id, title, summary, status)
           VALUES ($1,'rfx',$2,$3,$4,$5) RETURNING id`, [publicId, actor.organizationId, title, summary, lifecycle === "open" ? "active" : "draft"]);
      ...
      if (body.command === "respond-submit") {
        ...
        const response = await client.query<{ id: string }>(
          `INSERT INTO rfx_responses (rfx_record_id, respondent_organization_id, status, response_data, submitted_at)
           SELECT rr.id,$1,'submitted',$2::jsonb,now() FROM rfx_records rr WHERE rr.exchange_record_id=$3
           ON CONFLICT (rfx_record_id, respondent_organization_id) DO UPDATE SET status='submitted', response_data=excluded.response_data, submitted_at=now(), updated_at=now()
           RETURNING id`, [actor.organizationId, JSON.stringify(responseData), record.id]);
      ...
      if (body.command === "responses-matches") {
        const responses = await client.query<{ id: string; organization: string; status: string; submitted_at: string | null }>(
          `SELECT rsp.id, o.name AS organization, rsp.status, rsp.submitted_at
           FROM rfx_responses rsp JOIN rfx_records rr ON rr.id=rsp.rfx_record_id JOIN organizations o ON o.id=rsp.respondent_organization_id
           WHERE rr.exchange_record_id=$1 ORDER BY rsp.submitted_at DESC NULLS LAST`, [owned.id]);
        const matches = await client.query<{ public_id: string; title: string; score: number; status: string }>(
          `SELECT er.public_id, er.title, md.score, md.status FROM match_decisions md JOIN exchange_records er ON er.id=md.matched_exchange_record_id WHERE md.source_exchange_record_id=$1 ORDER BY md.score DESC NULLS LAST`, [owned.id]);
      ...
      if (body.command === "award-advance") {
        ...
        const selected = await client.query<{ id: string }>(
          `UPDATE rfx_responses rsp SET status='selected', updated_at=now()
           FROM rfx_records rr WHERE rsp.id=$1 AND rsp.rfx_record_id=rr.id AND rr.exchange_record_id=$2 RETURNING rsp.id`, [responseId, owned.id]);

...
const QUICK_PUBLICATION = new Set(["need", "select-rfx-type", "scope", "requirements", "schedule", "preview"]);
const GUIDED_PUBLICATION = new Set([...QUICK_PUBLICATION, "deliverables", "response-instructions", "pre-publication-validation"]);
const FORMAL_PUBLICATION = new Set([...GUIDED_PUBLICATION, "evaluation-criteria", "evaluation-governance", "approval-gates", "publication-readiness"]);
const RESPONSE_REQUIRED = new Set(["plan-response", "draft", "reused-profile-confirmation", "validate-compliance", "review"]);
const RESPONSE_WARNING = new Set(["assess-fit", "go-no-go", "qa-addenda", "collaborate"]);
...
export function isNodeComplete(node: RfxWorkflowNode, workspace: RfxWorkspace): boolean {
  ...
  return Boolean(node.fields?.length || node.checklist?.length || node.kind === "list");
}
...
export function summarizeChapter(node: RfxWorkflowNode, path: string[], workspace: RfxWorkspace): RfxChapterSummary {
  ...
  return { id: node.id, label: node.label, description: node.description, path, complete: completed.length, total: leaves.length, percent: leaves.length ? Math.round((completed.length / leaves.length) * 100) : 0, nextPath: next?.path, blockers: blockers.length };
}
...
export function progressForRoot(root: RfxWorkflowNode, workspace: RfxWorkspace) {
  ...
  return { complete, total: leaves.length, percent: leaves.length ? Math.round((complete / leaves.length) * 100) : 0 };
}
...
export function recommendRfxType(needStatement: string): RfxTypeRecommendation {
  ...
  if (containsAny(normalized, ["quote", "price", "lowest price", "unit price", "commodity"])) return { type: "RFQ", reason: "The need is primarily price- and specification-driven.", alternatives: ["Product Request", "RFP"], mode };
  if (containsAny(normalized, ["learn about", "information", "market research", "feedback", "understand the market"])) return { type: "RFI", reason: "The immediate goal is to gather information before making a purchase or selection.", alternatives: ["Sources Sought", "RFP"], mode };
  if (containsAny(normalized, ["sources sought", "who can", "capable suppliers", "capability statement"])) return { type: "Sources Sought", reason: "The need is focused on identifying capable organizations and market capacity.", alternatives: ["RFI", "Supplier Request"], mode };
  if (containsAny(normalized, ["subcontract", "subcontractor", "prime contractor"])) return { type: "Subcontractor Request", reason: "The request is seeking a subcontractor or delivery partner for a defined pursuit or project.", alternatives: ["Partner Request", "Supplier Request"], mode };
  if (containsAny(normalized, ["partner", "teaming", "joint pursuit", "collaborate"])) return { type: "Partner Request", reason: "The need is centered on finding a partner or teaming relationship.", alternatives: ["Subcontractor Request", "Sources Sought"], mode };
  if (containsAny(normalized, ["product", "equipment", "materials", "inventory", "supply"])) return { type: "Product Request", reason: "The request is centered on a product or tangible supply need.", alternatives: ["RFQ", "Supplier Request"], mode };
  if (containsAny(normalized, ["service", "consulting", "support", "maintenance", "training"])) {
    ...
    return complex ? { type: "RFP", reason: "Responders need to propose an approach, team, and delivery plan—not only a price.", alternatives: ["Service Request", "RFQ"], mode: formal ? "formal" : "guided" } : { type: "Service Request", reason: "The need is a defined service request that can use a lighter guided path.", alternatives: ["RFP", "RFQ"], mode };
  }
  return { type: "RFP", reason: "The need appears to require responders to explain how they would deliver the desired result.", alternatives: ["Service Request", "RFI"], mode: formal ? "formal" : "guided" };
}
...
function checkNode(root: RfxWorkflowNode, id: string, workspace: RfxWorkspace, label?: string): RfxPreflightItem {
  ...
  return { id, label: label ?? found.node.label, state: complete ? "complete" : "blocker", message: complete ? "Complete" : "Required work remains.", path: found.path };
}
...
function finishPreflight(items: RfxPreflightItem[]): RfxPreflightResult {
  ...
  return { complete, total: items.length, percent: items.length ? Math.round((complete / items.length) * 100) : 0, blockers, warnings, items, ready: blockers.length === 0 };
}
...
export function publicationPreflight(root: RfxWorkflowNode, workspace: RfxWorkspace) {
  ...
  if (qa && response && new Date(qa).getTime() >= new Date(response).getTime()) items.push({ id: "date-conflict", label: "Q&A and response dates", state: "blocker", message: "The response deadline must occur after the Q&A deadline.", path: [root.id, "build-scope", "schedule"] });
...
export function responsePreflight(root: RfxWorkflowNode, workspace: RfxWorkspace) {
  ...
  for (const id of RESPONSE_WARNING) {
    const found = findNodeAndPath(root, id);
    if (found && !isNodeComplete(found.node, workspace)) items.push({ id, label: found.node.label, state: "warning", message: "Recommended before final submission.", path: found.path });
  }
  return finishPreflight(items);
}
...
export function matchBreakdown(detail?: RfxDetail) {
  ...
  return { matched: requirements.filter((item) => item.profileState === "matched").length, confirm: requirements.filter((item) => item.profileState === "confirm").length, gap: requirements.filter((item) => item.profileState === "gap").length, total: requirements.length };
}

...
function scaleForZoom(zoom: number) {
  return Math.pow(2, (zoom - DEFAULT_ZOOM) / 2);
}

export function createDefaultMapView(): MapViewState {
  return {
    camera: {
      ...DEFAULT_MAP_VIEW.camera,
      center: { ...DEFAULT_MAP_VIEW.camera.center },
    },
...
export function projectCoordinate(coordinate: Coordinates, camera: MapCamera): ProjectedPoint {
  ...
  return {
    left,
    top,
    visible: left >= -8 && left <= 108 && top >= -8 && top <= 108,
  };
}
...
export function mapBoundsForCamera(camera: MapCamera): MapBounds {
  ...
  return {
    north: camera.center.lat + halfLat,
    south: camera.center.lat - halfLat,
    east: camera.center.lng + halfLng,
    west: camera.center.lng - halfLng,
  };
}
...
export function zoomMapCamera(camera: MapCamera, delta: number): MapCamera {
  return {
    ...camera,
    zoom: clamp(camera.zoom + delta, MAP_ZOOM_LIMITS.min, MAP_ZOOM_LIMITS.max),
  };
}
...
export function panMapCamera(camera: MapCamera, deltaX: number, deltaY: number, width: number, height: number): MapCamera {
  ...
  return {
    ...camera,
    center: {
      lat: clamp(camera.center.lat + deltaY * latPerPixel, 35.2, 38.4),
      lng: clamp(camera.center.lng - deltaX * lngPerPixel, -79.8, -74.2),
    },
  };
}
...
export function buildMarkerGroups(records: ExchangeRecord[], camera: MapCamera, selectedRecordId?: string): MapMarkerGroup[] {
  ...
  const singleton = (record: ExchangeRecord & { location: Coordinates }): MapMarkerGroup => ({
    id: record.id,
    records: [record],
    coordinate: record.location,
    projected: projectCoordinate(record.location, camera),
    cluster: false,
  });
  ...
  for (const record of candidates) {
    ...
    const match = groups.find((group) => {
      const center = averageCoordinate(group);
      const projected = projectCoordinate(center, camera);
      return Math.hypot(point.left - projected.left, point.top - projected.top) <= threshold;
    });
    if (match) match.push(record);
    else groups.push([record]);
  }

  const clustered = groups.map((group): MapMarkerGroup => {
    const coordinate = averageCoordinate(group);
    return {
      id: group.length === 1 ? group[0].id : `cluster:${group.map((record) => record.id).sort().join(",")}`,
      records: group,
      coordinate,
      projected: projectCoordinate(coordinate, camera),
      cluster: group.length > 1,
    };
  });

  return [...selected.map(singleton), ...clustered];
}

...
export function useUniversalSearchRuntime({enabled,lens,state}:{enabled:boolean;lens:ExchangeLens;state:unknown}){
  const stateKey=JSON.stringify(state);
  const normalizedState=useMemo(()=>normalizeSearchState(state),[stateKey]);
  ...
  async function loadMore(){if(!enabled||loadingMore||!runtime.nextCursor)return;setLoadingMore(true);try{const response=await searchExchangeThroughService(lens,normalizedState,runtime.nextCursor,30);setRuntime((current)=>{const seen=new Set(current.records.map((record)=>record.id));const appended=response.results.map((item)=>item.record).filter((record)=>!seen.has(record.id));return{...current,records:[...current.records,...appended],total:response.total,mapped:response.mapped,offMap:response.offMap,hasMore:Boolean(response.hasMore),nextCursor:response.nextCursor,facets:{...current.facets,...(response.facets??{})},status:"ready",initialized:true};});}finally{setLoadingMore(false);}}

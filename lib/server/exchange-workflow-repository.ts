...
import { assertExchangeActorMembership } from "./exchange-authorization";
...
const relationshipFor: Partial<Record<SharedWorkflowId, RelationshipKind>> = {
  save: "saved", watch: "watching", track: "tracking", follow: "following",
};
...
async function recordExecution(client: PoolClient, workflow: SharedWorkflowId, lens: ExchangeLens, source: WorkflowSource, actor: ResolvedExchangeActor, recordId: string, payload: Record<string, unknown>) {
  ...
  await client.query("INSERT INTO activity_events (event_name, actor_user_id, organization_id, exchange_record_id, payload) VALUES ($1,$2,$3,$4,$5::jsonb)", [eventNameFor[workflow], actor.userId, actor.organizationId, recordId, JSON.stringify(payload)]);
...
async function executeRelationship(client: PoolClient, workflow: SharedWorkflowId, actor: ResolvedExchangeActor, record: RecordRow) {
  ...
  await client.query(`INSERT INTO record_relationships (user_id, exchange_record_id, relationship_kind) VALUES ($1,$2,$3) ON CONFLICT (user_id, exchange_record_id, relationship_kind) DO UPDATE SET updated_at=now()`, [actor.userId, record.id, relationshipKind]);
...
async function executeShare(client: PoolClient, actor: ResolvedExchangeActor, record: RecordRow, lens: ExchangeLens) {
  ...
  const row = await client.query<{ id: string }>("INSERT INTO share_links (exchange_record_id, created_by_user_id, token_hash, audience) VALUES ($1,$2,$3,$4::jsonb) RETURNING id", [record.id, actor.userId, tokenHash, JSON.stringify({ type: "link" })]);
...
async function executeReferral(client: PoolClient, actor: ResolvedExchangeActor, record: RecordRow, lens: ExchangeLens, payload: Record<string, unknown>) {
  ...
  const referral = await client.query<{ id: string }>(`INSERT INTO referrals (sender_organization_id, recipient_organization_id, exchange_record_id, status, terms, created_by_user_id, source_lens, note) VALUES ($1,$2,$3,'proposed','{}'::jsonb,$4,$5,$6) RETURNING id`, [actor.organizationId, recipient.id, record.id, actor.userId, lens, note]);
  await client.query("INSERT INTO referral_events (referral_id, event_name, actor_user_id, payload) VALUES ($1,'ReferralCreated',$2,$3::jsonb)", [referral.rows[0].id, actor.userId, JSON.stringify({ sourceRecordPublicId: record.public_id, recipientOrganization: recipient.name })]);
...
async function executeCollaboration(client: PoolClient, workflow: "team" | "connect", actor: ResolvedExchangeActor, record: RecordRow, payload: Record<string, unknown>) {
  ...
  const result = await client.query<{ id: string }>(`INSERT INTO collaboration_requests (request_kind, exchange_record_id, sender_organization_id, recipient_organization_id, created_by_user_id, status, message) VALUES ($1,$2,$3,$4,$5,'requested',$6) RETURNING id`, [workflow === "team" ? "teaming" : "connection", record.id, actor.organizationId, record.organization_id, actor.userId, message]);
...
async function executeMatch(client: PoolClient, actor: ResolvedExchangeActor, record: RecordRow) {
  const result = await client.query<{ id: string; public_id: string; title: string; score: number }>(`WITH source AS (SELECT title, summary FROM exchange_records WHERE id=$1), ranked AS (SELECT er.id, er.public_id, er.title, ts_rank_cd(er.search_document, plainto_tsquery('english', source.title || ' ' || source.summary)) AS score FROM exchange_records er, source WHERE er.id <> $1) SELECT id, public_id, title, score FROM ranked WHERE score > 0 ORDER BY score DESC, title LIMIT 8`, [record.id]);
  for (const match of result.rows) {
    await client.query(`INSERT INTO match_decisions (source_exchange_record_id, matched_exchange_record_id, requested_by_user_id, score, rationale) VALUES ($1,$2,$3,$4,$5::jsonb) ON CONFLICT (source_exchange_record_id, matched_exchange_record_id, requested_by_user_id) DO UPDATE SET score=excluded.score, rationale=excluded.rationale`, [record.id, match.id, actor.userId, match.score, JSON.stringify(["PostgreSQL full-text relevance"])]);
  }
  return { matches: result.rows.map(({ id: _id, ...match }) => match) };
}

export async function executeSharedWorkflow(input: { workflow: SharedWorkflowId; lens: ExchangeLens; recordPublicId: string; source: WorkflowSource; actor: ResolvedExchangeActor; payload: Record<string, unknown>; }) {
  return withExchangeTransaction(async (client) => {
    await assertExchangeActorMembership(client, input.actor);
...
export async function getReferralPolicy(recipientOrganization: string, actor: ResolvedExchangeActor) {
  return withExchangeTransaction(async (client) => {
    await assertExchangeActorMembership(client, actor);
    const recipient = await findRecipient(client, recipientOrganization);
    if (!recipient) return null;
    const policy = await client.query<{ policy: Record<string, unknown>; fee: Record<string, unknown> }>("SELECT policy, fee FROM organization_referral_policies WHERE organization_id=$1", [recipient.id]);

-- RFxchange Universal Search runtime.
-- Apply after canonical RFx / Resources / Intelligence / Capabilities and shared-workflow migrations.

ALTER TABLE exchange_records ADD COLUMN IF NOT EXISTS geography_label text NOT NULL DEFAULT '';

-- Keep the generated document bounded to public Exchange record identity/content.
...
CREATE INDEX IF NOT EXISTS exchange_records_org_type_idx ON exchange_records(organization_id, record_type);
...
COMMENT ON TABLE saved_searches IS 'User-scoped Universal Search criteria and alert configuration; authorization is resolved from the RFxchange Identity session.';
COMMENT ON TABLE search_activity IS 'Search activity history and alert-evaluation events. It is not an analytics substitute for the canonical Exchange records.';
